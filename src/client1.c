    ...
    while (1)
    {
        ...
        while (SDL_PollEvent(&PlayEvent))
        {
            ...
            case SDL_MOUSEBUTTONUP:
                ...
                if (start)
                {
                    if (PlayEvent.button.x > 96 && PlayEvent.button.x < 768 && PlayEvent.button.y > 56 && PlayEvent.button.y < 799 && turn && !lose && !whether_replying) // 在棋盘内部的点击事件
                    {
                        whether_get_power = 0;
                        itoa(PlayEvent.button.x, x_new_red, 10); // 十进制（数字转字符串)
                        itoa(PlayEvent.button.y, y_new_red, 10); // 十进制（数字转字符串)
                        printf("%s\n%s\n", x_new_red, y_new_red);
                        int bytes_send_x = send(socket_server, x_new_red, 3, 0); // 把字符串放到socket_server里
                        int bytes_send_y = send(socket_server, y_new_red, 3, 0); // 把字符串放到socket_server里
                    ...
                    if (PlayEvent.button.x > 355 && PlayEvent.button.x < 551 && PlayEvent.button.y > 423 && PlayEvent.button.y < 469 && whether_replying && !lose) // 悔棋的点击事件
                    {
                        ...
                        for (int i = 0; i < 10; i++)
                            for (int j = 0; j < 9; j++)
                                chessboard_red_new[i][j] = chessboard_red_old[i][j];
                        turn = 0;
                    }
                    ...
                    switch (Judge(transfer_x(x_old_red), transfer_y(y_old_red), transfer_x(x_new_red), transfer_y(y_new_red), chessboard_red_new, 0))
                    {
                    case 1:
                        turn = 0;
                        red = 1;
                        Store(chessboard_red_new, chessboard_red_old);
                        int tempt = chessboard_red_new[transfer_y(y_old_red)][transfer_x(x_old_red)];
                        chessboard_red_new[transfer_y(y_old_red)][transfer_x(x_old_red)] = 0;
                        // 动画效果
                        for (int i = 0; i < 20; i++)
                        {
                            NoteRect.x = 84 + 78 * transfer_x(x_old_red) + i * (float)((78 * (transfer_x(x_new_red) - transfer_x(x_old_red))) / 20);
                            NoteRect.y = 47 + 78 * transfer_y(y_old_red) + i * (float)((78 * (transfer_y(y_new_red) - transfer_y(y_old_red))) / 20);
                        ...
                        chessboard_red_new[transfer_y(y_new_red)][transfer_x(x_new_red)] = (tempt % 2 || !(tempt % 12)) ? tempt : tempt - 1;
                        for (int i = 0; i < 3; i++)
                        {
                            x_new_red[i] = '\0';
                            x_old_red[i] = '\0';
                        ...
                        switch (JudgeWinOrLose(chessboard_red_new))
                        {
                        case 0:
                            lose = 0;
                            break;
                    ...
                    case 2:
                        ...
                        turn = 0;
                        Store(chessboard_red_new, chessboard_red_old);
                        chessboard_red_new[transfer_y(y_new_red)][transfer_x(x_new_red)] += (chessboard_red_new[transfer_y(y_new_red)][transfer_x(x_new_red)] % 2) ? 1 : -1;
                        for (int i = 0; i < 3; i++)
                        {
                            x_new_red[i] = '\0';
                            x_old_red[i] = '\0';
            ...
            if (FD_ISSET(socket_server, &read_fds)) // 接收黑方信息
            {
                if (lose)
                {
                    opp_ready = 1;
                ...
                if (start && !turn && !lose)
                {
                    int bytes_received = which_to_receive ? recv(socket_server, y_new_black, 3, 0) : recv(socket_server, x_new_black, 3, 0);
                    if (bytes_received < 1)
                    {
                        printf("%d\n", WSAGetLastError());
                    ...
                    printf("Received %d bytes from server: %.*s\n", bytes_received, bytes_received, which_to_receive ? y_new_black : x_new_black);
                    which_to_receive = which_to_receive ? 0 : 1;
                    if (!which_to_receive)
                    {
                        switch (Judge(8 - transfer_x(x_old_black), 9 - transfer_y(y_old_black), 8 - transfer_x(x_new_black), 9 - transfer_y(y_new_black), chessboard_red_new, 1))
                        {
                        case 1:
                            turn = 1;
                            // 存储新棋盘
                            Store(chessboard_red_new, chessboard_red_old);
                            int tempt = chessboard_red_new[9 - transfer_y(y_old_black)][8 - transfer_x(x_old_black)];
                            chessboard_red_new[9 - transfer_y(y_old_black)][8 - transfer_x(x_old_black)] = 0;
                            // 动画效果
                            for (int i = 0; i < 20; i++)
                            {
                                NoteRect.x = 84 + 78 * (8 - transfer_x(x_old_black)) + i * (float)((78 * (transfer_x(x_old_black) - transfer_x(x_new_black))) / 20);
                                NoteRect.y = 47 + 78 * (9 - transfer_y(y_old_black)) + i * (float)((78 * (transfer_y(y_old_black) - transfer_y(y_new_black))) / 20);
                                NoteRect.w = FrameSurface->w + 5;
                                NoteRect.h = FrameSurface->h + 5;
                                SDL_Texture *ChessPieceTexture = SDL_CreateTextureFromSurface(Renderer, NoteSurface[tempt - 1]);
                                SDL_RenderCopy(Renderer, ChessPieceTexture, NULL, &NoteRect);
                            ...
                            chessboard_red_new[9 - transfer_y(y_new_black)][8 - transfer_x(x_new_black)] = (tempt % 2 || !(tempt % 12)) ? tempt : tempt - 1;
                            for (int i = 0; i < 3; i++)
                            {
                                x_new_black[i] = '\0';
                                x_old_black[i] = '\0';
                        ...
                        case 2:
                            turn = 1;
                            Store(chessboard_red_new, chessboard_red_old);
                            chessboard_red_new[9 - transfer_y(y_new_black)][8 - transfer_x(x_new_black)] += (chessboard_red_new[9 - transfer_y(y_new_black)][8 - transfer_x(x_new_black)] % 2) ? 1 : -1;
                            for (int i = 0; i < 3; i++)
                            {
                                x_new_black[i] = '\0';
                                x_old_black[i] = '\0';
...
void PrintAll()
{
    ...
    if ((!start || self_ready) && !(self_ready && opp_ready))
        Paint_Ready();
    if (0 < chessboard_red_new[transfer_y(y_new_red)][transfer_x(x_new_red)] && chessboard_red_new[transfer_y(y_new_red)][transfer_x(x_new_red)] < 13)
        PaintTheFrame();
...
void PaintTheFrame()
{
    NoteRect.x = 84 + 78 * transfer_x(x_new_red);
    NoteRect.y = 47 + 78 * transfer_y(y_new_red);
    NoteRect.w = FrameSurface->w + 5;
    NoteRect.h = FrameSurface->h + 5;
...
void PaintTheChessPieces()
{
    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 9; j++)
        {
            if (chessboard_red_new[i][j] == 0)
                continue;
            else
            {
                NoteRect.x = 114 + j * 78 - 30;
                NoteRect.y = 77 + i * 78 - 30;
                NoteRect.w = NoteSurface[chessboard_red_new[i][j] - 1]->w + 6;
                NoteRect.h = NoteSurface[chessboard_red_new[i][j] - 1]->h + 6;
                SDL_Texture *ChessPieceTexture = SDL_CreateTextureFromSurface(Renderer, NoteSurface[chessboard_red_new[i][j] - 1]);
                SDL_RenderCopy(Renderer, ChessPieceTexture, NULL, &NoteRect);
                SDL_DestroyTexture(ChessPieceTexture);
            }
        }
...
void Again()
{
    for (int i = 0; i < 10; i++)
        for (int j = 0; j < 9; j++)
        {
            chessboard_red_new[i][j] = chessboard_red_copy[i][j];
            chessboard_red_old[i][j] = chessboard_red_copy[i][j];
        }
    whether_get_power = 0;
